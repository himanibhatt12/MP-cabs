		...
		foreach ($select1 as $fetch) {
			...
			foreach ($notifications as $notification) {
				if ($notification->type == "App\Notifications\RenewDriverLicence") {
					...
					$body = __('fleet.driver') . ": " . User::find($notification->data['vid'])->name . "\n" . __('fleet.driver_licence_expire') . " " . date('d-m-Y', strtotime($notification->data['msg'])) . ".";
					...
					$body = __('fleet.vehicle') . ": " . $vehicle->make . "-" . $vehicle->model . "-" . $vehicle->license_plate . "\n" . __('fleet.vehicle_insurance') . " " . date('d-m-Y', strtotime($notification->data['msg']));
					...
					$body = __('fleet.vehicle') . ": " . $vehicle->make . "-" . $vehicle->model . "-" . $vehicle->license_plate . "\n" . __('fleet.vehicle_licence') . " " . date('d-m-Y', strtotime($notification->data['msg']));
